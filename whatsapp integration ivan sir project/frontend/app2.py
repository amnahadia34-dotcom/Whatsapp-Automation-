...
language_code = "es"  # All your templates are Spanish
...
def send_template_message(token, phone_id, to, template, header_type, media_url):
    ...
    # Add header component if required
...
if send_button:
    ...
    if not access_token or not phone_number_id:
        ...
        numbers = [clean_number(n) for n in df.iloc[:, 0].dropna()]
        ...
        for i, number in enumerate(numbers):
            ...
            response, payload = send_template_message(
                access_token,
                phone_number_id,
                number,
                template_name,
